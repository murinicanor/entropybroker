...
void pool::stir(unsigned char *ivec, unsigned char *what, int n, unsigned char *temp_buffer, bool direction)
{
	...
	int ivec_offset = 0;
	BF_cfb64_encrypt(entropy_pool, temp_buffer, pool_size_bytes, &key, ivec, &ivec_offset, direction ? BF_ENCRYPT : BF_DECRYPT);
	memcpy(entropy_pool, temp_buffer, pool_size_bytes);
}

int pool::get_stir_size() const
{
	return 56; // BLOWFISH
}
...
int pool::add_entropy_data(unsigned char *entropy_data, int n_bytes_in)
{
	...
	while(n_bytes_in > 0)
	{
		...
		int cur_to_add = min(n_bytes_in, get_stir_size());

		stir(cur_ivec, entropy_data, cur_to_add, temp_buffer, true);
