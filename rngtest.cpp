...
static char RNGTEST_bit1cnt[256];

/* ringbuffer of 20000 bits */
static unsigned char RNGTEST_rval[20000/8];
/* point to current, ehr, thing */
static int RNGTEST_p;
/* number of bits in ringbuffer */
static int RNGTEST_nbits;
/* number of new bits after a long-test */
static int RNGTEST_nnewbits;

/* number of bits set to 1 (monobit test) */
static int RNGTEST_n1;

/* for poker test */
static int RNGTEST_pokerbuf[16];

/* where to store statistics */
rngtest_stats_t *prtst = NULL;
static char rngtest_stats[4096];
...
{
	snprintf(rngtest_stats, sizeof(rngtest_stats), "monobit: %d, poker: %d, longrun: %d, runs: %d",
		prtst -> monobit, prtst -> poker, prtst -> longrun, prtst -> runs);

	return rngtest_stats;
}
