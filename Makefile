...
CXXFLAGS+=-O3 -g3 -ggdb -march=native -mtune=native -DVERSION=\"${VERSION}\" $(LINT) $(DEBUG) -DCONFIG=\"${ETC}/entropy_broker.conf\" -DCACHE_DIR=\"${CACHE}\" -DPID_DIR=\"${PID}\" -DVAR_DIR=\"${VAR}\" -rdynamic $(PCSC_CFLAGS)
...
OBJSssc=server_smartcard.o utils.o kernel_prng_rw.o kernel_prng_io.o log.o error.o protocol.o server_utils.o auth.o my_pty.o users.o
...
all:
	...
	@echo
	@echo eb_server_cycle_count
	@echo
	@echo eb_server_smartcard
	@echo
...
eb_server_smartcard: $(OBJSssc)
	$(CXX) $(LINT) $(OBJSssc) $(LDFLAGS) `pkg-config --libs libpcsclite` -o eb_server_smartcard
